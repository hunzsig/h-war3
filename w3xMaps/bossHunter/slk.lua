...
for k, v in ipairs(heros) do
    ...
    ?>
    call SaveStr(hash_myslk, StringHash("heros"), StringHash("<?=v.Name?>"), "<?=addslashes(json.stringify(v))?>")
    call SaveStr(hash_myslk, StringHash("herosItems"), StringHash("<?=v.Name?>"), "<?=addslashes(json.stringify(hitem))?>")
    <?
end