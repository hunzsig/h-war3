...
library funcs
     ...
     * 特效 绑定单位
     */
    public function effectUnitLoc takes string effectModel,unit targetUnit returns nothing
    ...
    */
    ...
    public function setBattleStatus2Ready takes player whichPlayer returns nothing
        call setBattleStatus( STRING_STATUS[1] ,whichPlayer )
    endfunction
     //修改玩家战斗状态 -> 战斗中
    public function setBattleStatus2Battle takes player whichPlayer returns nothing
        call setBattleStatus( STRING_STATUS[2] ,whichPlayer )
    endfunction
     //修改玩家战斗状态 -> 已战死
    public function setBattleStatus2Death takes player whichPlayer returns nothing
        call setBattleStatus( STRING_STATUS[3] ,whichPlayer )
    endfunction
     //修改玩家战斗状态 -> 奋战中
    public function setBattleStatus2VeryBattle takes player whichPlayer returns nothing
        call setBattleStatus( STRING_STATUS[4] ,whichPlayer )
    endfunction
    //修改玩家战斗状态 -> 挂机中
    public function setBattleStatus2Idling takes player whichPlayer returns nothing
        call setBattleStatus( STRING_STATUS[5] ,whichPlayer )
    endfunction
    //修改玩家战斗状态 -> 选择英雄中
    public function setBattleStatus2ChoosingHero takes player whichPlayer returns nothing
        call setBattleStatus( STRING_STATUS[6] ,whichPlayer )
    endfunction
    //修改玩家战斗状态 -> 已离线
    public function setBattleStatus2Offline takes player whichPlayer returns nothing
        call setBattleStatus( STRING_STATUS[7] ,whichPlayer )
    endfunction
     ...
     * SET 失败句柄[单位]
     */
    public function setFailHandle_Unit takes unit u,string tips returns boolean
        local integer handleId
        if( u == null ) then
            return false
        endif
        set handleId = GetHandleId(u)
        call SaveUnitHandle(HASH_Fail,HS_ParentKey_Fail_Unit,handleId,u)
        call SaveStr(HASH_Fail,HS_ParentKey_Fail_Unit,(handleId+1),tips)
        call TriggerRegisterUnitEvent( gg_trg_fail_handle, u, EVENT_UNIT_DEATH )
        return true
    endfunction
     ...
     * CHECK失败句柄[单位]
     */
    public function checkFailHandle_Unit takes unit u returns boolean
        local integer handleId
        if( u == null ) then
            return false
        endif
        set handleId = GetHandleId(u)
        set u = LoadUnitHandle(HASH_Fail,HS_ParentKey_Fail_Unit,handleId)
        if( u == null ) then
            return false
        endif
        return true
    ...
    public function getFailHandle_Unit takes unit u returns string
        local integer handleId
        if( u == null ) then
            return ""
        endif
        set handleId = GetHandleId(u)
        return LoadStr(HASH_Fail,HS_ParentKey_Fail_Unit,(handleId+1))
    endfunction

    /**
     * UNSET 失败句柄[单位]
     */
    public function unsetFailHandle_Unit takes unit u returns boolean
        local integer handleId
        if( u == null ) then
            return false
        endif
        set handleId = GetHandleId(u)
        call SaveUnitHandle(HASH_Fail,HS_ParentKey_Fail_Unit,handleId,null)
        call SaveStr(HASH_Fail,HS_ParentKey_Fail_Unit,(handleId+1),null)
        return true
    endfunction
