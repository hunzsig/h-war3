globals
	...
	trigger Boss_Spell_Trigger_God = CreateTrigger()

	trigger Boss_Spell_Trigger_God_Bombs = CreateTrigger()

endglobals

library characterEnemySpellGod requires characterEnemySpellCrazy
	    ...
	    if( killer == null ) then
		    ...
		    loop
	            exitwhen(IsUnitGroupEmptyBJ(g) == true)
	                ...
	                set hunt = GetUnitState( u , UNIT_STATE_MAX_LIFE) * 0.18
	...
	private function action takes nothing returns nothing
		...
		if( GetUnitTypeId(boss) == Enemy_Config_Last_Final_boss ) then
			...
			//@90%几率触发无敌，冷却60，持续7.0
		    if( GetRandomInt(1,100) <= 90 ) then
				call characterEnemySpellAbstract_invincible( 60 , boss , 7.00 )
		    endif
			//@<85%>几率回避，冷却0.1秒
	        ...
	        elseif( damage > GetUnitState(boss, UNIT_STATE_MAX_LIFE)*0.15 ) then
	        	...
	        	call characterEnemySpellAbstract_avoid ( 0.1 , boss , 85 )
        	endif
			//@15%几率晕破吼，冷却8.5秒，范围1000，伤害750
			if( GetRandomInt(1,100) <= 15 and IsUnitType(damageSource, UNIT_TYPE_HERO) == true ) then
				call characterEnemySpellAbstract_breakHowl ( 8.5 , boss , 1000 , 750 )
			endif
			//@召唤小弟，40秒，内部分发
			...
			//@对召唤物造成5%生命伤害
		    if(IsUnitType(damageSource, UNIT_TYPE_SUMMONED) == true) then
		        call SetUnitLifeBJ( damageSource , GetUnitState(damageSource, UNIT_STATE_LIFE) - (I2R(DIFF)*0.05*GetUnitState(damageSource, UNIT_STATE_MAX_LIFE)))
		    endif
	        ...
	        elseif( damage > GetUnitState(boss, UNIT_STATE_MAX_LIFE)*0.15 ) then
	        	...
	        	call characterEnemySpellAbstract_avoid ( 0.25 , boss , 65 )
        	endif
		    //@10%几率晕破吼，冷却15.0秒，范围600，伤害350
		    if( GetRandomInt(1,100) <= 10 and IsUnitType(damageSource, UNIT_TYPE_HERO) == true ) then
				call characterEnemySpellAbstract_breakHowl ( 15.0 , boss , 600 , 350 )
			endif
			//@召唤小弟，60秒，内部分发
			call characterEnemySpellAbstract_callBrother ( 60 , boss )
		endif
		...
		if( IsUnitType(damageSource, UNIT_TYPE_HERO) == true ) then
			...
			if( GetUnitTypeId(boss) == 'n02Q' ) then		//锯裂机车
				//@20%几率触发锯裂机车JUMP，冷却12
			    if( GetRandomInt(1,100) <= 20 ) then
					call characterEnemySpellAbstract_jump( 12 , boss , damageSource , 20 , Effect_ImpaleTargetDust , bossDamegeBase*0.20 , Effect_red_shatter )
			    endif
			elseif( GetUnitTypeId(boss) == 'n02U' ) then	//守卫强暴者
				//@10%几率触发炸弹，冷却15 ， 数量5
			    if( GetRandomInt(1,100) <= 15 ) then
					call characterEnemySpellAbstract_summon( 5 , boss , 'n02V' , 7 , 125 , 5.00 , Boss_Spell_Trigger_God_Bombs )
			    endif
			elseif( GetUnitTypeId(boss) == 'n02R' ) then	//死神
				//@9%几率触发穿梭，冷却65 , 30次
			    if( GetRandomInt(1,100) <= 9 ) then
					call characterEnemySpellAbstract_shuttle( 65 , boss , 1000 , 45 , 30 , null , bossDamegeBase*0.08 , Effect_ReplenishManaCaster )
			    endif
			...
			elseif( GetUnitTypeId(boss) == 'n02S' ) then	//DarkEle
			    ...
			    if( GetRandomInt(1,100) <= 5 ) then
					call characterEnemySpellAbstract_canyingta( 75 , boss , 30 , 'o00S' , bossDamegeBase*0.11)
			    endif
			endif
			//BOSS
			if( GetUnitTypeId(boss) == 'n004' ) then		//巨石人
			    ...
			    if( GetRandomInt(1,100) <= 9 ) then
					call characterEnemySpellAbstract_canyingta( 45 , boss ,15 , 'o00N' , bossDamegeBase*0.07)
			    endif
			elseif( GetUnitTypeId(boss) == 'n00M' ) then	//怖残像狼
				//@9%几率触发狼咬，冷却18
			    if( GetRandomInt(1,100) <= 9 ) then
					call characterEnemySpellAbstract_leap(18 , boss , damageSource , 350 , 30 , null , bossDamegeBase*0.35 , Effect_Boold_Cut , false)
			    endif
			elseif( GetUnitTypeId(boss) == 'n03K' ) then	//提灯白牛
				//@7%几率触发白牛JUMP，冷却35
			    if( GetRandomInt(1,100) <= 7 ) then
					call characterEnemySpellAbstract_jump( 35 , boss , damageSource , 13 , Effect_CrushingWhiteRing , bossDamegeBase*0.35 , Effect_DarkLightningNova )
			    endif
			elseif( GetUnitTypeId(boss) == 'n01O' ) then	//军统领
				//@7%几率触发光明炮，8道，冷却60，距离1600，速度13，不重叠伤害
			    if( GetRandomInt(1,100) <= 7 ) then
					call characterEnemySpellAbstract_multiLeap(60, 'o00O' , boss , damageSource , 1600 , 13 , null , 8 , 45 , bossDamegeBase*0.40 , Effect_LightStrikeArray , false )
			    endif
			elseif( GetUnitTypeId(boss) == 'n01K' ) then	//食人大佬
				//@7%几率触发鬼巨人撞击，冷却65
			    if( GetRandomInt(1,100) <= 7 ) then
					call characterEnemySpellAbstract_charge(65 , boss , damageSource , SKILL_CHARGE_CRASH , 600 , 25 , Effect_ImpaleTargetDust , bossDamegeBase*0.65 , null , false )
			    endif
			elseif( GetUnitTypeId(boss) == 'n005' ) then	//群暴食尸鬼
				//@9%几率触发尸鬼冲击，冷却27
			    if( GetRandomInt(1,100) <= 9 ) then
					call characterEnemySpellAbstract_leap(27 , boss , damageSource , 350 , 45 , null , bossDamegeBase*0.45 , Effect_Boold_Cut , false)
			    endif
			elseif( GetUnitTypeId(boss) == 'n020' ) then	//鬼巨人
				//@9%几率触发鬼巨人撞击，冷却60
			    if( GetRandomInt(1,100) <= 9 ) then
					call characterEnemySpellAbstract_charge(60 , boss , damageSource , SKILL_CHARGE_FLY , 500 , 20 , Effect_ImpaleTargetDust , bossDamegeBase*0.60 , null , false )
			    endif
			...
			elseif( GetUnitTypeId(boss) == 'n00K' ) then	//炸弹魔
			    ...
			    if( GetRandomInt(1,100) <= 10 ) then
					call characterEnemySpellAbstract_summon( 12 , boss , 'n02V' , 4 , 125 , 5.00 , Boss_Spell_Trigger_God_Bombs )
			    endif
			...
			elseif( GetUnitTypeId(boss) == 'n00N' ) then	//鬼狼蛛
				//@15%几率触发狼蛛JUMP，冷却10
			    if( GetRandomInt(1,100) <= 15 ) then
					call characterEnemySpellAbstract_jump( 15 , boss , damageSource , 20 , Effect_HydraCorrosiveGroundEffect , bossDamegeBase*0.15 , Effect_GreatElderHydraAcidSpew )
			    endif
			...
			elseif( GetUnitTypeId(boss) == 'n040' ) then	//外道魔导师
				//@9%几率触发龙卷风，冷却19
			    if( GetRandomInt(1,100) <= 9 ) then
					call characterEnemySpellAbstract_chargeToken(19 , boss , 'o00T' , damageSource , SKILL_CHARGE_DRAG , 800 , 35 , null , bossDamegeBase*0.14 , null , false )
			    endif
			elseif( GetUnitTypeId(boss) == 'n02C' ) then	//狂斩刺客
				//@9%几率触发穿梭，冷却50 ， 15次
			    if( GetRandomInt(1,100) <= 9 ) then
					call characterEnemySpellAbstract_shuttle( 50 , boss , 1000 , 25 , 15 , null , bossDamegeBase*0.10 , Effect_ShadowAssault )
			    endif
			elseif( GetUnitTypeId(boss) == 'n02B' ) then	//飞蛇
				//@9%几率触发穿梭，冷却50 ，7次
			    if( GetRandomInt(1,100) <= 9 ) then
					call characterEnemySpellAbstract_shuttle( 50 , boss , 1000 , 20 , 7 , null , bossDamegeBase*0.18 , Effect_CrushingWaveBrust )
			    endif
			elseif( GetUnitTypeId(boss) == 'n03F' ) then	//深海海民
				//@9%几率触发海民冲击，冷却40
			    if( GetRandomInt(1,100) <= 9 ) then
					call characterEnemySpellAbstract_charge(40 , boss , damageSource , SKILL_CHARGE_DRAG , 800 , 6 , Effect_CrushingWaveDamage , bossDamegeBase*0.55 , null , false )
			    endif
			elseif( GetUnitTypeId(boss) == 'n02D' ) then	//破坏猛犸王
				//@9%几率触发猛犸王冲击，冷却35
			    if( GetRandomInt(1,100) <= 9 ) then
					call characterEnemySpellAbstract_charge(35 , boss , damageSource , SKILL_CHARGE_DRAG , 1000 , 11 , Effect_ImpaleTargetDust , bossDamegeBase*0.45 , null , false )
			    endif
			...
			if( GetUnitTypeId(boss) == 'n013' ) then		//红龙
			    ...
			    if( GetRandomInt(1,100) <= 12 ) then
					call characterEnemySpellAbstract_multiLeap(55, 'o00M' , boss , damageSource , 700 , 2 , null , 7 , 40 , bossDamegeBase*0.04 , Effect_Explosion , true)
			    endif
			elseif( GetUnitTypeId(boss) == 'n014' ) then	//黑龙
				//@12%几率触发殛寒领域，冷却60，范围650，持续3秒
			    if( GetRandomInt(1,100) <= 12 ) then
					call characterEnemySpellAbstract_multiPunish(60, boss, Effect_IceStomp , 650 , 3.00 , SKILL_PUNISH_TYPE_blue , bossDamegeBase*0.25 , Effect_FrostNovaTarget )
			    endif
			endif
		endif
	...
	public function init takes nothing returns nothing
    	call TriggerAddAction( Boss_Spell_Trigger_God , function action )
    	call TriggerAddAction( Boss_Spell_Trigger_God_Bombs , function action_Bombs )
	endfunction
