globals
    ...
    integer m1ShakeBull_spell_jianta = 'A074'
    integer m1ShakeBull_spell_tuwei = 'A062'
    integer m1ShakeBull_spell_zhendangbo = 'A052'
    integer m1ShakeBull_spell_canyingta = 'A081'
    integer m1ShakeBull_spell_zhenhandadi = 'A04R'

    integer m1ShakeBull_spell_zhendangbo_arrow = 'o005'
    integer m1ShakeBull_spell_canyingta_token = 'o00V'

endglobals

library m1ShakeBull requires m1Hero
	...
	public function tuwei takes nothing returns nothing
	    ...
	    local integer skillLv = GetUnitAbilityLevel(mover ,m1ShakeBull_spell_tuwei)
	    ...
	    local real hunt = I2R(Attr_Move[index]) + skillLv * 150.00
	...
	public function zhendangbo takes nothing returns nothing
	    ...
	    set arrow = funcs_createUnit ( GetOwningPlayer(u) , m1ShakeBull_spell_zhendangbo_arrow ,  loc ,  loc )
	...
	private function canyingtaCall takes nothing returns nothing
	    ...
	    if( shadowQtyInc < shadowQty ) then
	        ...
	        set u = funcs_createUnitFacing( GetOwningPlayer(triggerUnit) , m1ShakeBull_spell_canyingta_token ,loc , GetRandomReal(0.00,360.00))
	...
	public function canyingta takes nothing returns nothing
	    ...
	    local integer skillLv = GetUnitAbilityLevel( triggerUnit , m1ShakeBull_spell_canyingta )
...
library m1ShakeBullUse requires m1ShakeBull


	//Action - 释放技能
	private function action_spell_effect takes nothing returns nothing
		local integer abilityId = GetSpellAbilityId()
		if( abilityId == m1ShakeBull_spell_jianta ) then
			call m1ShakeBull_jianta()
		elseif( abilityId == m1ShakeBull_spell_tuwei ) then
			call m1ShakeBull_tuwei()
		elseif( abilityId == m1ShakeBull_spell_zhendangbo ) then
			call m1ShakeBull_zhendangbo()
		elseif( abilityId == m1ShakeBull_spell_canyingta ) then
			call m1ShakeBull_canyingta()
		elseif( abilityId == m1ShakeBull_spell_zhenhandadi ) then
			call m1ShakeBull_zhenhandadi()
		endif
