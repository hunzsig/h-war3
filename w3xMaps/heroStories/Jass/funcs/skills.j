library skills requires attribute
    ...
    public function shapeshiftCall takes nothing returns nothing
		...
		if(Global_SPELL_Talent_F_isLearn[index] == true) then
			call UnitRemoveAbility( u , Global_SPELL_Talent_F )
		endif
		if(Global_SPELL_Talent_C_isLearn[index] == true) then
			call UnitRemoveAbility( u , Global_SPELL_Talent_C )
		endif
		if(Global_SPELL_Talent_V_isLearn[index] == true) then
			call UnitRemoveAbility( u , Global_SPELL_Talent_V )
		endif
    ...
    public function leapCall takes nothing returns nothing
        ...
        local unit damageSource = funcs_getTimerParams_Unit(t,Key_Skill_UnitSource)
        ...
        if(hunt > 0) then
            ...
            call funcs_huntGroup(tmp_group,damageSource,hunt,HEffect,remove_group,FILTER_ENEMY)
    ...
    //剃
    public function leap takes unit mover, unit damageSource, location targetPoint,real speed,string Meffect,real hunt,real range,string Heffect,boolean isRepeat returns nothing
        ...
        call funcs_setTimerParams_Unit(t,Key_Skill_UnitSource,damageSource)
    ...
    private function avoidCallBack takes nothing returns nothing
        ...
        call funcs_console("handel:avoidCallBack END")
        call funcs_delTimer(t,null)
    endfunction
